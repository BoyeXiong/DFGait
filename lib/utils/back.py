...
def plot(x, colors):
    ...
    for i in range(4):
        ...
        txts.append(txt)
    plt.savefig('./nodecoupe.png', dpi=1000)
    plt.savefig('./nodecoupe.eps', format='eps', dpi=1000, 
                ) # 注意两个参数
    return f, ax, txts

tinydict = {'0058': 0, '0087': 1,
            '0405': 2, '0506': 3, '0695': 4,
            '2078': 5}
...
def evaluation_Gait3D(data, conf, probe_num, metric='euc'):
    ...
    results_path = conf["evaluator_cfg"]["results_path"]

    features, labels, cams, time_seqs = data['embeddings'], data['labels'], data['types'], data['views']
    # t-SNE可视化
    n = features.shape[0]
    sne_feature = features.reshape(n, -1)
    digits_final = TSNE(n_components=2, init='pca', perplexity=30).fit_transform(sne_feature)
    colors = np.empty(n, dtype = int, order = 'C') 
    for i in range(n):
        colors[i] = tinydict[labels[i]]
    # plt.scatter(digits_final[:, 0], digits_final[:, 1])
    # plt.savefig('./digits_tsne-pastel.png', dpi=300)
    plot(digits_final, colors)
