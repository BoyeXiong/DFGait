...
class Gait_Multimodel_GREW(BaseModel):
    ...
    def build_network(self, model_cfg):
        ...
        self.fc_bin = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.zeros(34, img_in_c[4], self.hidden_dim)))  #
        self.fc_bin1 = nn.Parameter(
            nn.init.xavier_uniform_(
                torch.zeros(34, ske_in_c[4], self.hidden_dim)))  
        
        # GCN
        self.GCN_sil = stDecoupe(in_channels=img_in_c[4],
                                 out_channels=img_in_c[4],
                                 kernel_size=[3, 1],
                                 num_nodes=17,
                                 )
        
        self.GCN_ske = stDecoupe(in_channels=ske_in_c[4],
                                 out_channels=ske_in_c[4],
                                 kernel_size=[3, 1],
                                 num_nodes=17,
                                 )
        self.space_token = nn.Parameter(torch.randn(1, 1, img_in_c[4]))
    ...
    def forward(self, inputs):
        ...
        sil = self.hp(outs4.permute(0, 1, 3, 4, 2), dim=3)  # (p, n, c, s)
        b, t, n, _ = sil.shape
